...
class auto():
    def __init__(self, wait_people="False", main=None, main2=None, ap=None):
        self.info = info()
        self.info.round = 1
        self.people = 0  # 人數
    ...
    def start(self, mode="quest"):
        if mode == "quest":
            if self.people == 2:
                while True:
                    self.start_2p(self.main, self.ap)
                    self.info.round += 1
            else:
                while True:
                    self.start_3p(self.main, self.main2, self.ap)
                    self.info.round += 1
        # else:
        #     if self.people == 2:
        #         while True:
        ...
        #     else:
        #         while True:
        #             self.event_3p(self.main, self.main2, self.ap)
        #             self.info.round += 1

    def start_2p(self, main, ap):
        ...
        self.info.ouput("[體力號]進入關卡")
        ap.standby(["quit_battle"], coordinates=[30, 60])
        self.info.ouput("[體力號]準備離開戰鬥")
        ap.standby(["yes"])
        ap.room_open = False
    ...
    def start_3p(self, main, main2, ap):
        self.info.display(end=True)
        ap.open_room()
    ...
    # def event_3p(self, main, main2, ap):
    #     self.info.display(end=True)
    #     ap.open_room()
    #     while main.finish or main2.finish:
    #         time.sleep(0.1)
    ...
    #     time.sleep(0.3)
    #     ap.standby("start_battle")
    #     self.info.ouput("[體力號]開始關卡")
    #     ap.standby("please_wait", tap=False)
    #     time.sleep(3)
    #     ap.device.shell("am force-stop air.com.gamania.worldflipper")
    #     time.sleep(0.3)
    #     self.info.ouput("[體力號]關閉遊戲")
    #     ap.device.shell(
    #         "am start -n air.com.gamania.worldflipper/air.com.gamania.worldflipper.AppEntry")
    #     self.info.ouput("[體力號]開啟遊戲")
    #     new_room = threading.Thread(target=ap.goto_event)
    #     self.info.ouput("[體力號]回到開房頁面中...")
    #     new_room.start()
    #     ap.room_open = False
    #     main_battle = threading.Thread(target=main.complete)
    #     main2_battle = threading.Thread(target=main2.complete)
